...
const char *modes[NUM_MODES] = { "Reverb", "Resonator",  "LO-FI", "MicroLooper"};
...
class LedsControl {
    ...
    void UpdateLeds() {
        ...
        case PRESET:
            versio.SetLed(DaisyHank::RGB_LED_1, mode == REV ? 1 : 0,0,0 );
            versio.SetLed(DaisyHank::RGB_LED_2, mode == RESONATOR ? 1: 0,0,0);
            versio.SetLed(DaisyHank::RGB_LED_3, mode == LOFI ? 1 : 0,0,0);
            versio.SetLed(DaisyHank::RGB_LED_4, mode == MLOOPER ? 1 : 0,0,0);
            
            break;

        case CV:
            
            versio.SetLed(DaisyHank::RGB_LED_1, 0,0,cvToCtrl == 0 ? 1 : 0 );
            versio.SetLed(DaisyHank::RGB_LED_2, 0,0,cvToCtrl == 1 ? 1 : 0);
            versio.SetLed(DaisyHank::RGB_LED_3, 0,0,cvToCtrl == 2 ? 1 : 0);
            versio.SetLed(DaisyHank::RGB_LED_4, 0,0,cvToCtrl == 3 ? 1 : 0);
            break;
        }
...
int main(void)
{
    ...
    lofi_rate_par.Init(versio.knob[DaisyHank::KNOB_1], sample_rate*4, sample_rate/16, lofi_rate_par.LOGARITHMIC);
...
{  
    ...
    switch (editmode)
    {
        case NORMAL:
            ...
            break;
        case PRESET:
            if(versio.s1.RisingEdge()) {
                firstTimeMode = false;
            }
            
            if(versio.s1.FallingEdge() && !firstTimeMode){
...
{  
    ...
    float ctrl1 = versio.GetKnobValue(DaisyHank::KNOB_1); //speed
    float ctrl2 = versio.GetKnobValue(DaisyHank::KNOB_2); //tone
    float ctrl3 = versio.GetKnobValue(DaisyHank::KNOB_3); //index
    float ctrl4 = versio.GetKnobValue(DaisyHank::KNOB_4); //regen
    ...
    float res_oct = 0;
    float res_tone = 1;
    float res_shimmer = 0.75;

    float lofi_rev_am = 0;
    float lofi_lpg_decay = 0;
    float lofi_lpg_am = 1;
    float lofi_lpg_cutoff = 20000;

    float looper_freeze = 0;
    float looper_drywet = 0.5;
    ...
    switch(mode)
    {
        case REV:
            ...
            reverb_lowpass = global_sample_rate*ctrl2 / 2.f;
            rev.SetLpFreq(reverb_lowpass);      
            reverb_shimmer = ctrl3;
            reverb_feedback = 0.8f + (std::log10(10 + ctrl1*90) -1.000001f)*0.4f;
            reverb_feedback_display = ctrl1*100;
            rev.SetFeedback(reverb_feedback);
            
            reverb_compression = rev_comp +0.5f;
        ...
        case RESONATOR:
            ...
            SelectResonatorOctave(res_oct);
            resonator_note = 12.0f + ctrl2 * 60.0f;
            resonator_note = static_cast<int32_t>(resonator_note); // Quantize to semitones


            resonator_tone = global_sample_rate*res_tone / 4.f;
            tone_freq = resonator_tone /2.f;
            //rev.SetLpFreq(resonator_tone);


            rev.SetLpFreq(resonator_tone*2.f);      
            reverb_shimmer = res_shimmer*2;
            reverb_feedback = 0.8f + (std::log10(10 + ctrl3*90) -1.000001f)*1.4f;
            ...
            reverb_compression = ctrl3*2 +0.5f;

            fonepole(resonator_current_regen,ctrl1, 0.008) ;
        ...
        case LOFI:
            ...
            lofi_cutoff = lofi_lpg_cutoff; 
            lofi_depth = ctrl2*2;     //index
            tonel.SetFreq(lofi_cutoff); 
            toner.SetFreq(lofi_cutoff);
            lofi_mod = (int)lofi_rate_par.Process();
            lofi_drywet = ctrl4*1.01; //IMPLEMENT

            reverb_lowpass = lofi_cutoff;
            rev.SetLpFreq(reverb_lowpass);
            reverb_feedback = 0.7f + (std::log10(10 + lofi_rev_am*90) -1.000001f)*0.3f;
            rev.SetFeedback(reverb_feedback);

            lofi_lpg_amount = lofi_lpg_am*lofi_lpg_am*3;
            lofi_lpg_decay = clamp((1-((std::log10(10 + lofi_lpg_decay*90) -0.999991f)*0.4f +0.6f  ) ) *0.05f, 0.0001, 0.99999);
            //Shimmer
            reverb_shimmer = 0.0f;
            reverb_compression = 0.5f;
            //DRY WET
            reverb_drywet = lofi_lpg_decay*0.8f;
        ...
        case MLOOPER:
            ...
            SelectLooperDivision(ctrl1,ctrl2);
            SelectLooperPlaySpeed(ctrl3,ctrl4);

            //mlooper_drywet = looper_drywet*1.01;
            mlooper_drywet = 0.5;
...
void LoadSettings(){
    const MultiversioSettings& mvsettings = settings.mvsettings();
    mode = mvsettings.mode;
    cvToCtrl = mvsettings.cvToCtrl;
}

void SaveSettings(){
    MultiversioSettings* mvsettings = settings.hank_mvsettings();
    mvsettings->mode = mode;
    mvsettings->cvToCtrl = cvToCtrl;
