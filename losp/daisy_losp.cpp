...
void DaisyLosp::Init(bool boost)
{
    // seed init
    seed.Configure();
    seed.Init(boost);
...
void DaisyLosp::SetHidUpdateRates()
{
    for(size_t i = 0; i < CONTROL_LAST; i++)
    {
...
void DaisyLosp::StartAudio(AudioHandle::InterleavingAudioCallback cb)
{
    seed.StartAudio(cb);
}

void DaisyLosp::StartAudio(AudioHandle::AudioCallback cb)
{
    seed.StartAudio(cb);
}

void DaisyLosp::ChangeAudioCallback(AudioHandle::InterleavingAudioCallback cb)
{
    seed.ChangeAudioCallback(cb);
}

void DaisyLosp::ChangeAudioCallback(AudioHandle::AudioCallback cb)
{
    seed.ChangeAudioCallback(cb);
}

void DaisyLosp::StopAudio()
{
    seed.StopAudio();
}

void DaisyLosp::SetAudioBlockSize(size_t size)
{
    seed.SetAudioBlockSize(size);
    SetHidUpdateRates();
}

size_t DaisyLosp::AudioBlockSize()
{
    return seed.AudioBlockSize();
}

void DaisyLosp::SetAudioSampleRate(SaiHandle::Config::SampleRate samplerate)
{
    seed.SetAudioSampleRate(samplerate);
    SetHidUpdateRates();
}

float DaisyLosp::AudioSampleRate()
{
    return seed.AudioSampleRate();
}

float DaisyLosp::AudioCallbackRate()
{
    return seed.AudioCallbackRate();
}

void DaisyLosp::StartAdc()
{
    seed.adc.Start();
}

void DaisyLosp::StopAdc()
{
    seed.adc.Stop();
}

void DaisyLosp::ProcessDigitalControls()
{
    encoder.Debounce();
}

void DaisyLosp::ProcessAnalogControls()
{
    for(size_t i = 0; i < CONTROL_LAST; i++)
    {
        controls[i].Process();
...
void DaisyLosp::SetLed(size_t idx, float red, float green, float blue)
{
    leds[idx].Set(red, green, blue);
}

void DaisyLosp::DelayMs(size_t del)
{
    seed.DelayMs(del);
}

void DaisyLosp::UpdateLeds()
{
    for(size_t i = 0; i < LED_LAST; i++)
    {
        leds[i].Update();
...
float DaisyLosp::GetKnobValue(int idx)
{
    return controls[idx].Value();
}

void DaisyLosp::UpdateExample()
{
    encoder.Debounce();
    SetLed(LED_LEFT,
           sw[SW_LEFT].Read() / 2.0,
