...
int main(void)
{
    ...
    filter_cutoff_l_par.Init(versio.knobs[DaisyVertigo::KNOB_0], 60, 20000, filter_cutoff_l_par.LOGARITHMIC);
    filter_cutoff_r_par.Init(versio.knobs[DaisyVertigo::KNOB_4], 60, 20000, filter_cutoff_r_par.LOGARITHMIC);

    delay_cutoff_par.Init(versio.knobs[DaisyVertigo::KNOB_1], 400, 20000, delay_cutoff_par.LOGARITHMIC);
    ...
    lofi_tone_par.Init(versio.knobs[DaisyVertigo::KNOB_2], 20, 20000, lofi_tone_par.LOGARITHMIC);
    lofi_rate_par.Init(versio.knobs[DaisyVertigo::KNOB_1], sample_rate*4, sample_rate/16, lofi_rate_par.LOGARITHMIC);
...
{  
    float blend = versio.GetKnobValue(DaisyVertigo::KNOB_0);
    float speed = versio.GetKnobValue(DaisyVertigo::KNOB_1); 
    float tone = versio.GetKnobValue(DaisyVertigo::KNOB_2);
    float index = versio.GetKnobValue(DaisyVertigo::KNOB_3);
    float regen = versio.GetKnobValue(DaisyVertigo::KNOB_4); 
    float size = versio.GetKnobValue(DaisyVertigo::KNOB_5);
    float dense = versio.GetKnobValue(DaisyVertigo::KNOB_6);

    float tone_freq;

    int sw1,sw2;

    switch(versio.sw[DaisyVertigo::SW_0].Read()) {
        case 0:
            sw1 = 1;
            break;
        case 1:
    ...
    switch(versio.sw[DaisyVertigo::SW_1].Read()) {
        case 0:
            sw2 = 3;
            break;
        case 1:
    ...
    switch(mode)
    {
        ...
        case RESONATOR:
            ...
            if (versio.s[DaisyVertigo::S_Tap].RisingEdge()){
...
versio.s[DaisyVertigo::S_Tap].Debounce();
