...
void Settings::Init(DaisyPinkman* hw) {
    hw_ = hw;
    LoadSettings();
    LoadStateSettings();
    //RestoreSettings();
...
{
    daisy::PersistentStorage<HwtestSettings> settings_storage(hw_->qspi);
...
{
    daisy::PersistentStorage<StateSettings> settings_storage(hw_->qspi);
