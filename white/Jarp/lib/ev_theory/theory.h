...
namespace ev_theory {
    ...
    uint8_t quantize(uint8_t, int, uint8_t);

    // @param semitone value
    // @param mode
    // @param mode root
    // @return 0-indexed scale degree of the given note (quantized)
    uint8_t noteToScaleDegree(uint8_t, int, uint8_t);

    // @param 0-indexed scale degree (<6)
    // @param mode
    // @param mode root
    // @return semitone value of that degree
    uint8_t scaleDegreeToNote(uint8_t, int, uint8_t);
    ...
    enum modes {
        ...
        Minor,
        Locri,  // Locrian
        PentMaj, // Pentatonic major
        PentMin, // Pentatonic Minor
        YoPent, // Yo Pentatonic
        MODES_LAST
    };

    struct ModeSemiTones {
        int num_notes;
        int notes[7];  
    };

    const ModeSemiTones modesemi[] = {
        {7,{0, 2, 4, 5, 7, 9, 11}}, // Major
        {7,{0, 2, 3, 5, 7, 9, 10}}, // Dorian
        {7,{0, 1, 3, 5, 7, 9, 10}}, // Phyrgi
        {7,{0, 2, 4, 6, 7, 9, 11}}, // Lydian
        {7,{0, 2, 4, 5, 7, 9, 10}}, // Mixo
        {7,{0, 2, 3, 5, 7, 8, 10}}, // Minor
        {7,{0, 1, 3, 5, 6, 8, 10}}, // Locri
        {5,{0, 2, 4, 7, 9}}, // PentMaj
        {5,{0, 3, 5, 7, 10}}, // PentMin
        {5,{0, 2, 5, 7, 9}} // YoPent
    };

    
    struct VoicingsScale {
        int num_notes;
        uint8_t notes[7];  
    };

    const VoicingsScale vscales[] = {
        {3,{1, 3 ,5}}, // Triad
        {4,{1, 3, 5, 8}}, // Triad+
        {4,{1, 3, 5, 7}}, // 7th
        {5,{1, 3, 5, 7, 8}}, // 7th+
