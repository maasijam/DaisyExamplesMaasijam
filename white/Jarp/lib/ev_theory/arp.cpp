...
Arp::Arp(int theMaxSteps, int thePattern){
    resetState();
...
void Arp::updateStep(){
    ...
    if (pattern == Random){
        ...
        traversal[traversalIndex] = step;
    } else {
        step = traversal[traversalIndex];
    }
    traversalLedIndex = traversalIndex;
...
int Arp::getTraversalIndex(){
    return traversal[traversalLedIndex];
}
