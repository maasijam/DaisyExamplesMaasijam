...
void UpdateEncoder()
{
  ...
  cur_page = (ui_page)(cur_page);
  if (cur_page >= LAST_PAGE) { cur_page = MIDI; }
  switch(cur_page)
  {
    case MIDI:
      // disable controls
      hw.SetRGBColor(RGB_LED_1,PURPLE);
      break;
    case GAIN_OUT:
      hw.SetRGBColor(RGB_LED_1,RED);
      break;
    case STIFF_BETA:
      hw.SetRGBColor(RGB_LED_1,GREEN);
      break;
    case STIFF_LFO:
      hw.SetRGBColor(RGB_LED_1,CYAN);
      break;
    case BETA_LFO:
      hw.SetRGBColor(RGB_LED_1,ORANGE);
      break;
    case IFC_MGF:
      hw.SetRGBColor(RGB_LED_1,BLUE);
      break;
    case IFC_LFO:
      hw.SetRGBColor(RGB_LED_1,WHITE);
      break;
    case AD:
      hw.SetRGBColor(RGB_LED_1,YELLOW);
      break;
    default: break;
  } 
...
void SetLedMode()
{
  switch(cur_mode) {
    case PING:
      ...
      break;
    case NOISE_ENV:
      hw.SetRGBColor(RGB_LED_2,PURPLE);
      break;
    case EXT:
      hw.SetRGBColor(RGB_LED_2,CYAN);
      break;
    case EXT_ENV:
      hw.SetRGBColor(RGB_LED_2,BLUE);
      break;
    case INHARM:
      hw.SetRGBColor(RGB_LED_2,YELLOW);
      break;
    case INHARM_NOISE:
      hw.SetRGBColor(RGB_LED_2,WHITE);
      break;
    default:
      break;
  }
...
int main(void)
{
	...
	knob1_lin.Init(hw.knob[KNOB_0], 0.0f, 1.0f, knob1_lin.LINEAR); 
	knob1_log.Init(hw.knob[KNOB_0], 0.0f, 1.0f, knob1_log.EXPONENTIAL); 
	knob2_lin.Init(hw.knob[KNOB_1], 0.0f, 1.0f, knob2_lin.LINEAR); 
	knob2_log.Init(hw.knob[KNOB_1], 0.0f, 1.0f, knob2_log.EXPONENTIAL); 
	knob1_lin.Process();
        knob1_log.Process();
        knob2_lin.Process();
        knob2_log.Process();

	hw.SetRGBColor(RGB_LED_1,PURPLE); // MIDI MODE
	hw.SetRGBColor(RGB_LED_2,OFF); // PING MODE
