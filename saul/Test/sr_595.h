#pragma once
#ifndef DSY_DEV_SR_595_LOCAL_H
#define DSY_DEV_SR_595_LOCAL_H
...
const size_t numMatrixStates
    = 24; /**< Maximum Number of chained devices Connect device's QH' pin to the next chips serial input*/
...
class ShiftRegister595
{
  ...
  */
    ...
    */
    ...
    \param state A true state will set the output HIGH, while a false state will set the output LOW.
    */
    void SetMatrix(uint8_t idx, bool state);
  ...
  private:
    ...
    uint8_t  stateMatrix_[numMatrixStates];
    size_t   num_devices_;
