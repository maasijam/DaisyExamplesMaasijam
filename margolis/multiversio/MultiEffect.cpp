...
class LedsControl {
    ...
    void SwitchOffLed(int idx) {
         versio.SetRgbLeds(static_cast<LeddriverLeds>(idx),0,0,0);
    };
    ...
    void UpdateLeds() {
        ...
        for (int i = 0; i < 4; i++) {
            versio.SetRgbLeds(static_cast<LeddriverLeds>(i),base_color[i][0],base_color[i][1],base_color[i][2]);
            if (times[i] > 0) {
                times[i]--;
                versio.SetRgbLeds(static_cast<LeddriverLeds>(i), flash_color[i][0],flash_color[i][1],flash_color[i][2]);
            }
        }
        versio.UpdateLeds();
    };
...
int main(void)
{
    ...
    filter_cutoff_l_par.Init(versio.knob[KNOB_1], 60, 20000, filter_cutoff_l_par.LOGARITHMIC);
    filter_cutoff_r_par.Init(versio.knob[KNOB_5], 60, 20000, filter_cutoff_r_par.LOGARITHMIC);

    delay_cutoff_par.Init(versio.knob[KNOB_2], 400, 20000, delay_cutoff_par.LOGARITHMIC);
    ...
    lofi_tone_par.Init(versio.knob[KNOB_3], 20, 20000, lofi_tone_par.LOGARITHMIC);
    lofi_rate_par.Init(versio.knob[KNOB_2], sample_rate*4, sample_rate/16, lofi_rate_par.LOGARITHMIC);
...
{  
    float blend = versio.GetKnobValue(KNOB_1);
    float speed = versio.GetKnobValue(KNOB_2); 
    float tone = versio.GetKnobValue(KNOB_3);
    float index = versio.GetKnobValue(KNOB_4);
    float regen = versio.GetKnobValue(KNOB_5); 
    float size = versio.GetKnobValue(KNOB_6);
    float dense = versio.GetKnobValue(KNOB_7);

    float tone_freq;

    int sw1,sw2;
    if(versio.SwitchState(S0A)){
        sw1 = 0;
    } else if(versio.SwitchState(S0B)) {
        sw1 = 2;
    } else {
        sw1 = 1;
    }
    
    if(versio.SwitchState(S1A)){
        sw2 = 0;
    } else if(versio.SwitchState(S1B)) {
        sw2 = 6;
    } else {
        sw2 = 3;
    }
